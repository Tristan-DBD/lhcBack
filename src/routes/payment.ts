 ...
 */
router.post('/toggle', authenticate, authorize('COACH'), async (req: AuthRequest, res: Response) => {
    try {
        const { userId, year, month } = req.body

        const updatedPayment = await PaymentService.toggleMonth(userId, year, month)
